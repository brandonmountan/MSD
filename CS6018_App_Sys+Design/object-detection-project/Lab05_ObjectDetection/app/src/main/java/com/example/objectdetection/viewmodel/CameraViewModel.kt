        ...
        viewModelScope.launch {
            try {
                ...
                if (result.isSuccess) {
                    ...
                    android.util.Log.e("CameraViewModel", "Upload failed: $errorMsg") // ADD THIS
                    Toast.makeText(context, "Photo saved locally. Upload failed: $errorMsg", Toast.LENGTH_LONG).show() // UPDATE THIS
                }
            } catch (e: Exception) {
                _cameraState.value = _cameraState.value.copy(isCapturing = false)
