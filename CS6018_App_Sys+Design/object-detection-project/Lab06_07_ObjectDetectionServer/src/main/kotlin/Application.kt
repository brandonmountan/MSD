...
@Serializable
data class UploadSuccessResponse(val success: Boolean, val photoId: String)
...
object Storage {
    ...
    // Use relative path instead of /app/photos
    private val photosDir = File("photos").apply {
        mkdirs()
        println("📁 Photos directory created at: ${this.absolutePath}")
    }
    ...
    fun registerUser(username: String, password: String): Boolean {
        ...
        if (success) {
            ...
            userDir.mkdirs()
            println("📁 Created user directory: ${userDir.absolutePath}")
        }
        return success
    }
    ...
    fun savePhoto(username: String, photoId: String, bytes: ByteArray) {
        ...
        if (!userDir.exists()) {
            userDir.mkdirs()
            println("📁 Created user photo directory: ${userDir.absolutePath}")
        }

        val photoFile = File(userDir, "$photoId.jpg")
        ...
        routing {
            ...
            post("/login") {
                ...
                if (Storage.validateUser(credentials.username, credentials.password)) {
                    ...
                    println("🔑 Created token for ${credentials.username}: $token") // ADD THIS
            ...
            post("/logout") {
                ...
                call.respond(HttpStatusCode.OK, SuccessResponse(true, "Logged out"))
            }
            ...
            post("/photos/upload") {
                ...
                try {
                    ...
                    if (photoBytes != null) {
                        ...
                        call.respond(UploadSuccessResponse(success = true, photoId = photoId))
                    } else {
                        call.respond(HttpStatusCode.BadRequest, ErrorResponse("No photo data"))
                    }
                } catch (e: Exception) {
